...
The chat-start triage (`.agents/rules/dev-workflow-activation.md`) determines the mode for each conversation:
...
In **both** modes, every code change must satisfy the quality rules in `.agents/rules/quality.md` and the checklists below.
