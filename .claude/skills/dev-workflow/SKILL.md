...
## Step 8.5: Plan completeness gate

After verify and **before** creating the PR, check if a `code-review` skill is available at `.cursor/skills/code-review/SKILL.md` or `.claude/skills/code-review/SKILL.md`.

If it exists, read that `SKILL.md` and follow **Gate: Plan completeness**. Do not launch `code-review-plan` a second time — the Gate section owns the Task. Honor its verdict (Critical incomplete → do not create the PR, return to implementation; `no-plan` → warn and continue; complete → Step 9).
...
If the skill exists, run it as the **orchestrator** in this conversation (do not wrap the whole skill in one Task):

1. Read the skill file.
2. Skip Phase 1. Set review mode to `github`. Start at Phase 1.2 with the PR number, owner, and repo already known from the workflow.
3. Follow the skill from there: it launches specialist subagents in parallel (named `subagent_type` when available, else `generalPurpose` plus the agent file). Specialists return findings only. The parent posts one pending review.
4. Display the Phase 5 summary to the user (link to PR, number of inline comments posted by severity).

If the orchestrator fails or times out, inform the user that the automated review could not be completed and suggest running it manually later (e.g., "review this PR").
...
### 12.1 Fetch unresolved review threads
...
### 12.4 Handle questionable comments
...
- **Fix it as suggested**: treat as an agreed comment (12.3).
...
If yes, treat as agreed (12.3). If no, reply acknowledging the nit and resolve the thread.

### 12.6 Commit, push, and re-review
...
3. Return to 12.1 to check for new unresolved threads.

### 12.7 Loop termination
...
2. Skip Phase 1. Set review mode to `github`. Start at Phase 1.2 with the known PR number, owner, and repo. Follow the skill as the orchestrator (specialist subagents + fallback; do not wrap the whole skill in one Task).
